...
// Sprint 19.5 — Lesson "Attachments" (models/Lesson.js) covering the
// brief's full list (PDF/Worksheet/Images/ZIP/Reference Documents) with one
// upload endpoint. Combines the allowlists of the four savers above rather
// than adding a fifth near-duplicate saveProtectedFile() core call — still a
// fixed, closed allowlist (not "any file type"), per saveProtectedDocument's
// existing comment on why an unbounded allowlist would defeat validation.
...
export async function saveProtectedAttachment(request, subdir, { maxSizeBytes = 20 * 1024 * 1024 } = {}) {
  return saveProtectedFile(request, subdir, {
    maxSizeBytes,
    allowedTypes: ALLOWED_ATTACHMENT_TYPES,
    typeErrorMessage: 'Only images, PDFs, Word/PowerPoint/Excel documents, or ZIP archives are allowed',
    extFor: (filename, mimeType) => {
      const fromName = path.extname(filename || '').toLowerCase();
      if (fromName) return fromName;
      if (mimeType === 'application/pdf') return '.pdf';
      if (mimeType === 'application/zip' || mimeType === 'application/x-zip-compressed') return '.zip';
      return EXT_BY_IMAGE_MIME[mimeType] || EXT_BY_DOCUMENT_MIME[mimeType] || '.bin';
    },
  });
}
