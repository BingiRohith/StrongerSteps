...
export async function verifyOtp({ verificationId, otp }) {
  ...
  // Sprint 19.1B: expose which identifier was just proven so the route
  // handler can upsert/link the reusable VerifiedLead identity
  // (lib/verifiedLead.js) — this service stays OTP-mechanics-only and
  // doesn't import the lead layer itself, keeping it dependency-free for
  // any future caller that only wants the download token.
  return {
    ok: true,
    downloadToken,
