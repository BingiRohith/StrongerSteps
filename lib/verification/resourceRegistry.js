...
export const RESOURCE_TYPES = {
  ...
  lesson: {
    ...
    // fileKind: 'video' | 'pdf' | 'image' | 'attachment-<index>' | 'body-image-<index>'
    getFile: (doc, fileKind) => {
      ...
      // Sprint 19.5 — inline images inserted into `body` by the lesson rich
      // text editor. Same indexed-fileKind pattern as `attachment-<index>`.
      if (fileKind?.startsWith('body-image-')) {
        const index = Number(fileKind.slice('body-image-'.length));
        return doc.bodyImages?.[index] || null;
      }
      return doc.image;
    },
    subdirFor: (fileKind) => {
      ...
      if (fileKind?.startsWith('body-image-')) return 'lessons-body-images';
