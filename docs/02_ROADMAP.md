...
| §11 | Team page: organizational tree (Roots=Founders, Branches=Departments, Leaves=Members) | **Done** (Sprint 14) | `models/Team.js` gained `department` + self-ref `parentMember` (cycle-validated); `lib/teamHierarchy.js` derives tree level at read time (unlimited nesting, never hardcoded). About page's flat grid replaced with `components/team/OrgTree.js` (responsive desktop connector-chart / mobile collapsible list, Name/Department/Position search). Admin CRUD extended in place — no second Team module. |
...
5. ~~**Team org-tree layout**~~ (§11) — done in Sprint 14 (`department` + self-ref `parentMember` on `models/Team.js`, derived tree levels, responsive Organization Tree component, existing CMS data reused as-is).
