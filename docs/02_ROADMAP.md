...
| §9 | Products Module | **Done** for CRS's "already implemented" list. Sprint 12.5 (not itself in the CRS — see the CRS-assumptions note below) redesigned the public page into a marketplace layout (dynamic sidebar filters, server-side search/sort/pagination) and added an optional `brand` field. Sprint 18 replaced the closed 3-value `category` enum with a fully admin-managed `ProductCategory` CMS (`/admin/product-categories`) — CRS §2's "if the client should be able to change it, it belongs in the Admin Panel" now applies to category too. Future roadmap items (inventory, checkout, coupons, shipping, orders, payments) — **not started**. `stockStatus` (in-stock/out-of-stock) is a simple flag, not real inventory tracking. |
...
| §11 | Team page: organizational tree (Roots=Founders, Branches=Departments, Leaves=Members) | **Done** (Sprint 14, redesigned same day per client feedback — see `docs/13_DECISIONS.md`; mobile responsiveness closed Sprint 17) | `models/Team.js` gained `department`, self-ref `parentMember` (cycle-validated), and `xPosition`/`yPosition`. About page's flat grid replaced with `components/team/OrgTree.js` — a hand-illustrated tree (`TeamTreeIllustration.js`) with members placed by admin-set coordinates (`/admin/team/tree` drag editor), `parentMember` drawing a connector line rather than driving layout. Name/Department/Position search. Admin CRUD extended in place — no second Team module. The illustration only worked down to tablet width; Sprint 17 added `components/team/MobileOrgTree.js`, a collapsible hierarchy list, as the `<768px` fallback. |
...
| §15 | Admin Dashboard manages Blogs/Infographics/Products/Team/Memberships/Programs/Recipes/Events/Pricing/Categories/Media | **Partial** | Blogs/Infographics/Products/Team/Memberships/Programs(Events)/Recipes done. Categories (the generic Blog-only one) is still API-only, no management UI — Recipe Categories (Sprint 13) and Product Categories (Sprint 18) each have their own full management UI (see [07_ADMIN_MODULES.md](07_ADMIN_MODULES.md)). No dedicated Media library — uploads are per-module folders, not a reusable/browsable library. |
...
| §20 | Design principles: Green + Terracotta palette, senior-friendly, accessible | **Aligned, one open finding** | Verified — [`tailwind.config.js`](../tailwind.config.js) defines `primary` (#2F6B4F, green) and `accent` (#E59530, terracotta/orange). Sprint 17 audited WCAG contrast site-wide: `ink`/`muted`/`primary` all pass AA against `bg`; `accent-dark` on `bg` (used by the `Eyebrow` component, every section label) is 3.61:1, below the 4.5:1 AA threshold for its 14px bold text. Not changed — see `docs/13_DECISIONS.md`'s Sprint 17 entry (a brand-color change needs client sign-off, not a "polish" fix). |
...
7. ~~**Production hardening & Version 1.0**~~ — done in Sprint 17: full regression pass, Team Tree mobile responsiveness, SEO (`robots.js`/`sitemap.js`/metadata/favicon), security (login rate limiting), accessibility (dialog roles + a logged contrast finding), and a real performance fix (`/admin/homepage` code-split). No CRS content-module scope in this sprint — see `CHANGELOG.md`'s Sprint 17 entry for the full list.
8. ~~**UI polish, Product Categories CMS & homepage interaction**~~ — done in Sprint 18: `ProductCategory` CMS (closes the last CRS §2 gap in the Products module), a real header overflow bug fix, a responsive filter sidebar, homepage micro-animations, and a CMS-ready Courses card redesign (Knowledge Center, still no CRUD). Not itself a CRS §-numbered item — a UI/UX polish sprint layered on top of already-CRS-complete modules; see `CHANGELOG.md`'s Sprint 18 entry.

All CRS §4–§14 content-module gaps identified in this doc's original gap
analysis are now closed, and the application has completed a production-
hardening pass (Sprint 17) and a UI/UX polish pass (Sprint 18) — see
[10_SPRINT_HISTORY.md](10_SPRINT_HISTORY.md).
Remaining CRS items (§16 Payments, §17
