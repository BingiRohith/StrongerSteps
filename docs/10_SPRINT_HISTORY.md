...
| 11 | Membership CMS | 2026-07-07 | CRS §8. `Membership` model + full admin CRUD (`/admin/membership`, `active`/`inactive` lifecycle, individually reorderable benefits via `BenefitsEditor.js`) + `app/join/page.js` swapped from Sprint 10's hardcoded array to a live MongoDB fetch, with a friendly empty state and configurable badge/theme/CTA. `scripts/seedMembership.mjs` migrates the 3 previously-hardcoded plans. Latest sprint as of this doc. |
