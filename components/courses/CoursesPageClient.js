...
import { Loader2, Search, ChevronLeft, ChevronRight, GraduationCap } from 'lucide-react';
import CourseCard from './CourseCard';

// Search by title/instructor/category/tags/difficulty (CRS-equivalent
// search-field list from the Sprint 19.2 brief) is server-side via
// GET /api/courses's `search` (title/instructor/description text index)
// and `difficulty`/`category`/`tag` params — mirrors
// components/recipes/RecipesPageClient.js, consolidated into one file
// (category nav + toolbar + grid + pagination) rather than five separate
// ones, since Courses' filter surface is smaller than Recipes'.
...
export default function CoursesPageClient({ initialCourses, initialPagination, initialFilters, facets }) {
  ...
  const fetchCourses = useCallback(
    async (nextFilters, { syncUrl = true } = {}) => {
      ...
      try {
        ...
        if (syncUrl) router.replace(`/courses?${qs}`, { scroll: false });
      } catch (err) {
        setError('Could not load courses right now. Please try again.');
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    ...
    fetchCourses(next, { syncUrl: false });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams]);

  function updateFilters(patch) {
    fetchCourses({ ...filters, ...patch, page: patch.page ?? 1 });
  }
    ...
    <div className="mx-auto max-w-content px-6 py-12 md:py-16">
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm text-muted">{pagination.total} course{pagination.total === 1 ? '' : 's'}</span>
        </div>
      </div>
