...
export default function ProductCategoryForm({ categoryId, initialData }) {
  ...
  const [form, setForm] = useState(() => ({
    ...EMPTY_CATEGORY,
    ...initialData,
    icon: { ...EMPTY_CATEGORY.icon, ...initialData?.icon },
  }));
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(
        isEdit ? `/api/admin/product-categories/${categoryId}` : '/api/admin/product-categories',
        {
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
        ...
        <div className="rounded-xl2 border border-line bg-surface p-5 sm:p-6">
          ...
          <div className="mt-1.5 flex items-center gap-2">
            <span className="text-sm text-muted">/products?category=</span>
            <input
              id="slug"
              type="text"
              value={form.slug}
          ...
          <textarea
            ...
            placeholder="A short description (not currently shown publicly)"
      ...
      <div className="space-y-6">
        <div className="rounded-xl2 border border-line bg-surface p-5 sm:p-6">
          ...
          <p className="mt-1 text-xs text-muted">Only active categories appear on the public Products page.</p>
          ...
          <input
            ...
            className="mt-1.5 w-full rounded-lg border border-line bg-white px-3.5 py-2.5 text-sm text-ink outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
          />
          <p className="mt-1 text-xs text-muted">Lower numbers appear first in the category filter</p>
        </div>

        <div className="rounded-xl2 border border-line bg-surface p-5 sm:p-6">
          <h3 className="mb-3 font-display text-sm font-bold text-primary-dark">Category icon</h3>
          <p className="mb-3 -mt-2 text-xs text-muted">
            Optional — falls back to a generic icon on the Products page if left empty.
          </p>
          <ImageUploadField
            value={form.icon}
            onChange={(icon) => update('icon', icon)}
            uploadUrl="/api/admin/product-categories/upload"
          />
        </div>
      </div>
    </div>
