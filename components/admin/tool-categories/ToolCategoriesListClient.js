...
// Mirrors components/admin/resource-categories/ResourceCategoriesListClient.js
// exactly (same list/filter/reorder/toggle/delete pattern), pointed at the
// Tool Categories admin API.
const STATUS_TABS = [
  { value: '', label: 'All' },
...
export default function ToolCategoriesListClient() {
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/tool-categories?${params.toString()}`);
  ...
  async function updateCategory(id, body) {
    const res = await fetch(`/api/admin/tool-categories/${id}`, {
      method: 'PUT',
  ...
  async function toggleActive(category) {
    ...
    try {
      const res = await fetch(`/api/admin/tool-categories/${category._id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await fetch(`/api/admin/tool-categories/${deleteTarget._id}`, { method: 'DELETE' });
      const data = await res.json();
      if (res.ok && data.success) {
    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-xl font-bold text-primary-dark">Tool Categories</h2>
        ...
        <Link
          href="/admin/tool-categories/new"
          ...
          <div className="flex flex-col items-center px-6 py-16 text-center">
            ...
            <p className="mt-1 max-w-xs text-sm text-muted">
              {search || isActive
                ...
                : 'Get started by adding your first tool category.'}
            </p>
            {!search && !isActive && (
              <Link
                href="/admin/tool-categories/new"
            ...
            {sortedCategories.map((category, index) => (
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-1.5 self-end sm:self-auto">
                  ...
                  <Link
                    href={`/admin/tool-categories/${category._id}/edit`}
                    title="Edit"
