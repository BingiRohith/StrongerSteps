...
// Mirrors components/admin/resource-categories/ResourceCategoryForm.js exactly.
...
export default function ToolCategoryForm({ categoryId, initialData }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(
        isEdit ? `/api/admin/tool-categories/${categoryId}` : '/api/admin/tool-categories',
        {
      ...
      router.push('/admin/tool-categories');
      router.refresh();
    } catch (err) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
        ...
        <div className="rounded-xl2 border border-line bg-surface p-5 sm:p-6">
          ...
          <input
            ...
            placeholder="Assessments & Calculators"
          ...
          <div className="mt-1.5 flex items-center gap-2">
            <span className="text-sm text-muted">/tools?category=</span>
            <input
              id="slug"
              type="text"
              value={form.slug}
          ...
          <textarea
            ...
            placeholder="A short description shown on the tools category navigation"
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl2 border border-line bg-surface p-5 sm:p-6">
          <h3 className="mb-3 font-display text-sm font-bold text-primary-dark">Icon</h3>
          <p className="mb-3 -mt-2 text-xs text-muted">
            Optional — falls back to a generic icon on the public Tools page when unset.
          </p>
          <ImageUploadField
            value={form.icon}
            onChange={(icon) => update('icon', icon)}
            uploadUrl="/api/admin/tool-categories/upload"
          />
        </div>
      </div>
    </div>
