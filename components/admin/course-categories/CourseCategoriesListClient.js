...
// Mirrors components/admin/recipe-categories/RecipeCategoriesListClient.js
// exactly (same list/filter/reorder/toggle/delete pattern), pointed at the
// Course Categories admin API.
const STATUS_TABS = [
  { value: '', label: 'All' },
...
export default function CourseCategoriesListClient() {
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/course-categories?${params.toString()}`);
  ...
  async function toggleActive(category) {
    ...
    try {
      const res = await fetch(`/api/admin/course-categories/${category._id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await fetch(`/api/admin/course-categories/${deleteTarget._id}`, { method: 'DELETE' });
      const data = await res.json();
      if (res.ok && data.success) {
    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-xl font-bold text-primary-dark">Course Categories</h2>
            ...
            {sortedCategories.map((category, index) => (
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-1.5 self-end sm:self-auto">
                  ...
                  <Link
                    href={`/admin/course-categories/${category._id}/edit`}
                    title="Edit"
