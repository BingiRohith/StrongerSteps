...
// Mirrors components/admin/course-categories/CourseCategoryForm.js exactly.
...
export default function ResourceCategoryForm({ categoryId, initialData }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(
        isEdit ? `/api/admin/resource-categories/${categoryId}` : '/api/admin/resource-categories',
        {
      ...
      <div className="space-y-6 lg:col-span-2">
        ...
        <div className="rounded-xl2 border border-line bg-surface p-5 sm:p-6">
          ...
          <input
            ...
            placeholder="Assessments & Checklists"
          ...
          <div className="mt-1.5 flex items-center gap-2">
            <span className="text-sm text-muted">/resources?category=</span>
            <input
              id="slug"
              type="text"
              value={form.slug}
          ...
          <textarea
            ...
            placeholder="A short description shown on the resources category navigation"
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl2 border border-line bg-surface p-5 sm:p-6">
          ...
          <p className="mb-3 -mt-2 text-xs text-muted">
            Optional — falls back to a generic icon on the public Resources page when unset.
          </p>
          <ImageUploadField
            value={form.icon}
            onChange={(icon) => update('icon', icon)}
            uploadUrl="/api/admin/resource-categories/upload"
          />
        </div>
      </div>
    </div>
