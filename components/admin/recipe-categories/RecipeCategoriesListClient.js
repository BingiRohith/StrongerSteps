...
export default function RecipeCategoriesListClient() {
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/recipe-categories?${params.toString()}`);
  ...
  async function updateCategory(id, body) {
    const res = await fetch(`/api/admin/recipe-categories/${id}`, {
      method: 'PUT',
  ...
  async function toggleActive(category) {
    ...
    try {
      const res = await fetch(`/api/admin/recipe-categories/${category._id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await fetch(`/api/admin/recipe-categories/${deleteTarget._id}`, { method: 'DELETE' });
      const data = await res.json();
      if (res.ok && data.success) {
    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-xl font-bold text-primary-dark">Recipe Categories</h2>
        ...
        <Link
          href="/admin/recipe-categories/new"
          ...
          <div className="flex flex-col items-center px-6 py-16 text-center">
            ...
            <p className="mt-1 max-w-xs text-sm text-muted">
              {search || isActive
                ...
                : 'Get started by adding your first recipe category.'}
            </p>
            {!search && !isActive && (
              <Link
                href="/admin/recipe-categories/new"
            ...
            {sortedCategories.map((category, index) => (
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-1.5 self-end sm:self-auto">
                  ...
                  <Link
                    href={`/admin/recipe-categories/${category._id}/edit`}
                    title="Edit"
